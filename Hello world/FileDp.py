...
def encode_type_B(opcode, operands):
    reg1, imm = operands.split()
    imm = int(imm[1:])  
    encoded = opcodes[opcode] + '0' + registers[reg1] + format(imm, '07b')
    return encoded
...
def encode_type_E(opcode, operands):
    mem_addr = operands
    encoded = opcodes[opcode] + '0000' + format(mem_addr, '07b')
    return encoded
...
while True:
    ...
    if opcode in type_C and opcode != 'mov':
