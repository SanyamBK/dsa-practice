...
void testCase()
{
    ...
    // sort v based on second element in decreasing order, if second element is same then sort based on first element in increasing order
    ...
    Sort(c);
    // find index of first element greater than Min
    
    
    ll ans = 0;    
    for (auto it = lower_bound(all(c), Min); it != c.end(); it++) {
        int ing = *it;
        forloop (j, 0, n) {
            if (v[j].fi <= ing) {
                int full_cycles = (ing - v[j].fi) / (v[j].fi - v[j].se) + 1;
                ans += 2 * full_cycles;
                ing = ing - full_cycles * v[j].fi + full_cycles * v[j].se;
            }
        }
        *it = ing;
