...
#define PQminll priority_queue<ll, vll, greater<ll>> // pqminll
...
void testCase() {
    ...
    forloop(i, 0, n) {
        ...
        forloop(j, i + 1, n) {
            ...
            if (it != st.end()) xorArray.push_back(a[i] ^ *it);
            if (it != st.begin()) xorArray.push_back(a[i] ^ *prev(it));
        }
    }

    sort(xorArray.begin(), xorArray.end());
    print(xorArray[k - 1]);
}
