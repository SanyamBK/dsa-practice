...
bool checkDown(int i, int j, mi &a)
{
    if (a[i+1][j] < a[i][j]) {
            return false;
        }
...
bool checkLeft(int i, int j, mi &a)
{
    if (a[i][j-1] < a[i][j]) {
            return false;
        }
...
bool checkRight(int i, int j, mi &a)
{
    if (a[i][j+1] < a[i][j]) {
            return false;
        }
...
void testCase()
{
    ...
    forloop(i, 1, n+1)
    {
        ...
        {
            bool isGreater = true;
            if (a[i][j] <= a[i-1][j]) isGreater = false;
            if (a[i][j] <= a[i+1][j]) isGreater = false;
            if (a[i][j] <= a[i][j-1]) isGreater = false;
            if (a[i][j] <= a[i][j+1]) isGreater = false;
