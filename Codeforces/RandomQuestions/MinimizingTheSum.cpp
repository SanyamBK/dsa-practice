...
void testCase()
{
    ...
    // vll dp(n+1, INT_MAX);
    // dp[0] = accumulate(all(a), 0);
    int sumAll = accumulate(all(a), 0);
    ...
    forloop (i, 0, n) {
        int changeByLeft = 0;
        int changeByRight = 0;
        if (i > 0) changeByLeft = -a[i] + a[i-1];
        if (i < n-1) changeByRight = -a[i] + a[i+1];
        int change = min(changeByLeft, changeByRight);
        pq.push({change, i});
    }
    forloop (i, 0, k) {
        if (pq.empty()) break;
        ...
        if (idx > 0 && a[idx-1] > a[idx]) { 
            changeByLeft = a[idx] - a[idx-1];
            pq.push({changeByLeft, idx-1});
        }
        if (idx < n-1 && a[idx+1] > a[idx]) {
            changeByRight = a[idx] - a[idx+1];
            pq.push({changeByRight, idx+1});
        }
    }
    print(sumAll);
}
