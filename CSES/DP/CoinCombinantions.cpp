...
ll coinCombinations(vi &coins, int x, vll &dp) {
    if (x == 0) return 1;  // There is one way to make sum 0: using no coins.
    if (x < 0) return 0;   // No way to make a negative sum.
    if (dp[x] != -1) return dp[x]; // Return already computed result.

    ll ways = 0;
    for (int coin : coins) {
        ways = (ways + coinCombinations(coins, x - coin, dp)) % mod;
    }
    return dp[x] = ways;
}
...
void testCase()
{
    ...
    //     } 
    // }
    ll ans = coinCombinations(coins, x, dp);
    print(ans);
