...
int soln(vector<vector<int>> &dp, vector<int> &coins, int x, int n)
{
    if (n == 0)
        return dp[n][x] = (x % coins[n] == 0);
    if (x == 0)
        return dp[n][x] = 1;
    if (dp[n][x] != -1)
        return dp[n][x];
    int pick = (x >= coins[n]) ? soln(dp, coins, x - coins[n], n) : 0;
    int notpick = soln(dp, coins, x, n - 1);
    return dp[n][x] = (pick + notpick) % MOD;
}
